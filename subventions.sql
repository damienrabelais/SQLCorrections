...
-- La fonction SUM ignore les valeurs à « null »… aussi la somme pour « Education » qui devrait être égale à « 0 »… elle égale à « null ». 
-- Pour remplacer « null » par « 0 » on utilisera la fonction COALESCE comme suit : 

SELECT LibelleType, COALESCE(SUM(Montant), 0)
FROM TYPESUBVENTION
LEFT OUTER JOIN ENVELOPPE ON (TYPESUBVENTION.CodeType = ENVELOPPE.CodeType)
GROUP BY TYPESUBVENTION.LibelleType

--La fonction COALESCE renvoie le premier de ses arguments qui n'est pas nul. Null n'est retourné que si tous les arguments sont null.  Elle s’écrit comme suit, d’une manière générale : 
--COALESCE(valeur1, valeur2,.., valeurn)
--Dans l’exemple précédent, COALESCE(SUM(Montant), 0),  on a COALESCE(SUM(Montant), 0) = COALESCE(null, 0) -> Premier élément non « null » = 0 ! 
--On pourra également répondre à cette question comme suit : 

SELECT LibelleType, SUM(Montant)
FROM ENVELOPPE
INNER JOIN TYPESUBVENTION ON (ENVELOPPE.CodeType = TYPESUBVENTION.CodeType)
GROUP BY TYPESUBVENTION.LibelleType
  UNION
SELECT LibelleType, 0
FROM TYPESUBVENTION
WHERE CodeType Not In (
    SELECT CodeType
    FROM ENVELOPPE
